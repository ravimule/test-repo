$postModule = angular.module('postModule', ['ui.filters','pagination', 'ngDialog', 'jkuri.datepicker' ]);
...
$postModule.controller('PostController',function($scope, $http, ngDialog, $timeout){
	...
	var url = base_path+'home/ajax.php';
   ...
   $scope.editTemplate = function (user) {
    ...
    $scope.imageTemplate = function (user) {
    	$scope.image = user;
    	ngDialog.open({
		    template: 'largeimage',
	    	...
	    	if(data.success){ 
	    		if( $scope.editMode ){
	    			...
	    			$scope.post.users[$scope.index].designation = $scope.tempUser.designation;
	    		 	...
	    		 	$scope.post.users[$scope.index].birthDate = $scope.tempUser.birthDate;
	    		}else{ 
	    			$scope.post.users.push({
		    			...
		    			birthDate: $scope.tempUser.birthDate
		    		});
	    		}
	    		$scope.messageSuccess(data.message);
	    		//$scope.userForm.$setPristine();		//reset the form
	    		//$scope.tempUser = {};
	...
	$scope.sortUser = function(user){
		$scope.post.sortUser = user; 
		/*$http({
	      method: 'post',
	      url: url,
	      data: $.param({ 'sortUser' : user, 'type' : 'sort_user' }),
	      headers: {'Content-Type': 'application/x-www-form-urlencoded'}
	    }).
	    success(function(data, status, headers, config) {
	    	if(data.success){
	...
	$scope.sortAscDesc = function(user){
		$scope.post.sortAscDesc = user; 
	}
	...
	$scope.doVote = function(rowid){
		$http({
	      method: 'post',
	      url: url,
	      data: $.param({ 'row_selected' : rowid, 'type' : 'like' }),
	      headers: {'Content-Type': 'application/x-www-form-urlencoded'}
	    }).
	    success(function(data, status, headers, config) {
	    	if(data.success){
	    		$scope.post.likestatus = data.likestatus;
	    		console.log($scope.post.likestatus);
	    	}else{
	    		$scope.messageFailure(data.message);
	    	}
	    });		
